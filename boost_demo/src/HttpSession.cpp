...
HttpSession::~HttpSession() { std::cout << "Connection closed" << std::endl; }
...
void HttpSession::DownloadRealFile(const std::string &file_path) {
  // 打开文件
...
void HttpSession::SendFileChunk(std::ifstream file, std::streamsize file_size) {
    auto self(shared_from_this());
...
void HttpSession::DownloadVirtualFile(const size_t file_size) {
  // 解析文件大小
...
void HttpSession::SendVirtualData(size_t sent_bytes) {
  auto self(shared_from_this());
